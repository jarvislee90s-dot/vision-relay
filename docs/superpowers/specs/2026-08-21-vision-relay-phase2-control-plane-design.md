...
| 页面 | 承载需求 | 要点 |
|---|---|---|
| 总览 | R2/R8 | ① 状态横幅：**滑动式路由开关**（启停服务+接线）、刷新（便捷动作，自动对账默认在跑）、**诊断报告**（只读；「需要你」区有待办时入口给出提示）。② 三个 harness 拓扑卡：**竖排链路、动态渲染**——路由关则本代理节点置灰「已旁路」、连线右移直连下一跳；工具也关则再右移直连真实上游；卡上显示激活供应商及其 base_url（§5 优先级）。③ 卡片「详情」抽屉（默认折叠）：接管组合快照、本 harness 的 relay（来源/匹配模型/停用转发）、配置文件路径（含系统打开入口；不做行号定位——2026-08-23 收窄）——转发与工具看板并入此处，不设独立区块。④ **「自动处理」区**（被动通知：自动抢回、自动修复、新模型已实测标注、连通性自动测试——全是绿条只读，点开看事件）；**「需要你」区**（仅缺 key 补填一类）。⑤ **诊断报告弹层**（只读）：能自动修的已直接修并记录；仅无解项（快照读不到且服务起不来）才出现选择。漂移由对账自动收敛，**不设手动抢回按钮** |
