...
每个已知工具一条档案：名字 + 默认端口 + **支持的 harness 矩阵** + 生成的 relay 模板 + **激活供应商读取方式**。注意 CC Switch 同时覆盖 Claude Code 与 Codex，且**两者的路由开关都是逐 harness 的**（CC Switch 可只对 codex 开路由而 claude 直连；端口在线 ≠ 路由了某个 harness）——某 harness 的路由工具归属**必须逐 harness 以配置文件证据判定**（2026-08-26 用户裁决；实测事故：CC Switch 仅路由 codex 时，claude 卡也显示「直连 cc-switch :15721」）：

1. **配置文件端口指纹（第一证据）**：该 harness 自己的 base_url 指向 15721 → CC Switch 干的；指向 57321 → Codex++ 干的（`classify_base_url` 的 ownership）。路由关与本代理接管中漂移（工具趁 8787 在跑改写配置）都落在这里——对账会抢回，显示先如实；
2. **接管态用快照 second_hop**：base_url 已指向本代理时无从判断，用接管快照记录的 second_hop（接管时刻从配置文件读到的归属）；快照缺失/无 second_hop → 一层直连，不猜；
3. **端口在线探测只回答「工具进程活着吗」**（供应商提示、relay 层两层转发、离线回落用），**永不作为归属证据**；旧「dossier 全局矩阵 + 在线」推断已废弃。
...
- **记录只存一个字段：输入模态，三态**——`纯文本`（用户填写或探测得出）、`支持图片`（用户填写或探测得出）、`未标注`（空：既没探测结论也没用户填写）。上下文长度、推理档位等富元数据**不采集**（本项目用不上）。
- **取值优先级：用户覆盖 > 实测探测 > 目录建议 > 未标注（空）**。外部静态目录（models.dev 风格）降级为**标注建议来源**，且只取其输入模态字段。目录建议 = 内置 fnmatch 精确表 + 大小写不敏感正则启发式（正向 vision/vl/4o/omni/gpt-4/claude/gemini/glm-4v/llava/pixtral/kimi 等；负向 tts/asr/voice/whisper/embedding/rerank 词边界优先，安全侧；借鉴 dsh-image-vision guessVision，2026-08-25）。
- **可验证探针（实测探测）**：对每个 (provider, 模型) 用已配 key 发一次最小带图请求（红色 8×8 PNG，base64 内嵌各协议原生图片字段）。**纯接收判定**（2026-08-25 决策，参考 dsh-image-vision「检测」机制；旧「问颜色、答对才算」三档校验废弃——思考模型把小 token 预算烧在 thinking 块上吐不出正文，答案校验非确定且问句示例会泄漏答案）：**200**（带图请求被接受）→ 支持图片；**报错且报文含「不支持」类模态语义** → 纯文本（主力信号：不识图直接报错）；**超时 / 401 / 404 / 5xx / 报错但无模态语义**（含图片格式类报错）→ 不下结论，回落目录建议。已知残余风险：静默吞图模型（200 收图但模型没看到）会被判支持图片——由 user 标注优先（探测不覆盖用户值）+ 行内手工切换兜底。成本为每模型一次最小调用，结果按 (provider, 模型) 缓存、可重测；没配 key 的 provider 显示「未测」。可选「从上游拉取模型清单」（用已配 key 调 `/v1/models`，只补模型 ID 清单；回环/被抑制 relay 不拉但在 skipped 里带原因，GUI 据此解释「清单在工具自己的界面查看」）。探针只在该 (provider, 模型) 组合首次出现时自动执行一次，结果作为待确认值推给用户（向导②/模型能力页）——用户过目后无论原样保存还是修改，即为用户意图（source=user），此后不再被自动探测覆盖；重测仅由用户显式触发（2026-08-23 决策）。
- **空状态（未标注）的运行行为 = 一个全局开关**（设置·服务折叠区，默认走识图）：ON「按纯文本处理，走 VLM 转述」= 安全侧（绝不报错，但真视觉模型会白费转述 token）；OFF「直通」= 省 token，但真纯文本模型会报错。**UI 文案即开关两侧的代价说明**——这就是「误标代价」文案的最终形态；也是首次向导保留「过目」步骤的原因（实测结果已就绪，用户只改猜错的）。
...
| 页面 | 承载需求 | 要点 |
|---|---|---|
| 总览 | R2/R8 | ① 状态横幅：**滑动式路由开关**（启停服务+接线）、刷新（便捷动作，自动对账默认在跑）、**诊断报告**（只读；「需要你」区有待办时入口给出提示）。② 三个 harness 拓扑卡：**竖排链路、动态渲染**——工具节点按 §5 逐 harness 归属渲染（配置文件指纹 → 接管态快照 second_hop；无归属证据不画工具跳）；本代理节点仅在「路由开且接管态」为实路径，其余（路由关 / harness 直指工具端口 / 直连态）一律置灰「已旁路」；harness 直指工具时工具是真实下一跳（工具离线则如实标「不可达」，不虚构回落）；接管态工具离线才四跳回落一层直连；卡上显示激活供应商及其 base_url（§5 优先级）。③ 卡片「详情」抽屉（默认折叠）：接管组合快照、本 harness 的 relay（来源/匹配模型/停用转发）、配置文件路径（含系统打开入口；不做行号定位——2026-08-23 收窄）——转发与工具看板并入此处，不设独立区块。④ **「自动处理」区**（被动通知：自动抢回、自动修复、新模型已实测标注、连通性自动测试——全是绿条只读，点开看事件）；**「需要你」区**（仅缺 key 补填一类）。⑤ **诊断报告弹层**（只读）：能自动修的已直接修并记录；仅无解项（快照读不到且服务起不来）才出现选择。漂移由对账自动收敛，**不设手动抢回按钮** |
| 模型能力 | R1/R9 | **三元组表格**（harness × provider × 模型，provider 自动填充）+ 当前标注 + 依据（实测/目录）+ **实测列**（接受=支持图片 / 拒图=纯文本 / 未测，对齐 §5 纯接收判定）+ 操作（切换、重测）；「探测全部未测模型」与「从上游拉取模型清单（可选）」按钮；**查看与覆盖**定位——进来就是标注完的表，只在猜错时改，一次保存生效。**非当前激活供应商行默认折叠**（2026-08-25：展开仍可切换标注；重测禁用——不经工具路由、无可达探测路径）；重测防连点 + 行内 spinner + 状态行轻提示（仅不可达弹窗附 reason）；「探测全部未测」= 前端逐行驱动当前激活供应商的无缓存行（实时进度 N/M、实测列逐行刷新、结束弹汇总：支持图片/纯文本/无结论/不可达计数与首个不可达原因）；拉清单空结果解释回环原因（清单在工具自己的界面查看）。扫描矩阵来自工具档案只读读取（cc-switch SQLite `providers` 表 / Codex++ `settings.json` `relayProfiles`，含供应商名、base_url、模型清单、is_current/activeRelayId，`is_current` 随 models-scan 透出；密钥字段与 `modelVlm` 一律不读——后者用户裁决不准）；两工具均未装（直连态）时按 harness 配置 base_url 域名推导供应商名（已知域名映射表 + 主机名兜底）。能力标注键统一为该供应商名，legacy/`?` 影子桶读兜底、写清除。探测目标解析：在线工具端口 → harness 自身直连上游（live→snapshot，key 按 key_ref 位置取）→ relay 直连；无目标/无结论经 `target_found`+`reason` 透出，GUI 弹窗可见。GUI 调 core 子进程统一 120s 超时（挂死变明确报错，不再永远「探测中」） |
...
| 设置 | R1/R5 + 全局配置 | **VLM**（唯一必配卡：模型名称 / base URL / API key 带表头 + **显隐按钮**（👁 按需经 `vlm-secret` 动词回显明文 key——『输出不带 key』的唯一刻意豁免；离开本页自动恢复隐藏）+ 保存 + 可选测试）→ **按 harness 分组状态表**（跟随全局 ✓ / 自定义，点开才展开自定义四件套，默认零配置）；**外观**（语言：跟随系统/中文/English，可见）；**折叠区·识图提示词**（Tier1/Tier2 可编辑保存、一键恢复默认；测试四模式：Tier1×默认/自选、Tier2×默认/自选，走生产调用路径，可选自定义测试图（PNG/JPEG/WebP/GIF、单图 ≤10 MiB、读取中禁用测试按钮；仅本次诊断用——不写配置、不留痕识图记录、不跨窗口持久化，最终是否接受由所配置 VLM 服务决定）；**折叠区·服务与高级**（全部默认值：监听 127.0.0.1:8787、管理的 harness 自动检测已安装者、**未标注模型默认开关（走识图=安全 / 直通，§5）**、自动对账开·5 秒、识图留痕 7 天·仅本机、版本与契约）。relay 管理（停用/补 key）在总览详情抽屉，GUI 不提供手工新建 relay（§5） |
...
2. **能力值术语与存储**：新值 `image`，旧值 `vision` 读兼容；能力记录按 (harness, provider, 模型) 三元组存储、**只存输入模态一个字段、三态**（纯文本 / 支持图片 / 未标注；上下文与推理档位不采集）；旧按组扁平结构读兼容迁移；外部目录仅为标注建议来源；**探针结果按 (provider, 模型) 缓存**、可重测（§5）。
3. **接管组合快照与备份**：见 §5（每 harness 只存最新组合，不做历史清单；用户自定义回退档案已由「按崩溃前意图自动推导」取代，若仍有需求留三期）。
4. **识图提示词可自定义**：Tier1/Tier2 默认提示词可在 GUI 编辑、保存、一键恢复默认；识图管线读取当前生效值；测试面板支持默认/自选提示词 × Tier1/Tier2 四种组合，测试图可选用户自选图片（走同一调用路径；仅诊断用，不写配置、不留痕）。
5. **relay 压制名单**：用户停用自动探测生成的 relay = 显式意图，记入压制名单，自动探测不再加回。**数据面选路对其全层跳过**（2026-08-26 故障复盘收严：残留坏值 `https://x` 的 direct 条目已被用户停用，选路仍三层选中它，Claude Code 全线 502 且无法自救——「停用转发」必须名副其实：停用是显式意图，优先级最高，指纹精确命中也不得越过）。配套两件：①数据面按 proxy.json mtime **热加载**（控制面动词在独立进程写盘，服务不重启即生效——否则停用/补 key 对运行中的流量不可见）；②fail-open 502 与 `proxy_error` 日志带 `relay=<name> → <base_url>`（坏条目一眼定位，不再只报 "proxy internal error"）。
...
| **P1** | 模态探测误判/成本 | 纯接收判定（2026-08-25）：200 即支持图片、报错含不支持语义即纯文本、含糊（超时/401/404/5xx/格式报错）不下结论回落目录。残余：静默吞图（200 收图但模型没看到）会误判支持图片——接受此风险（思考模型使答案校验不可行），由 user 标注优先 + 行内切换兜底；成本 = 每模型一次最小调用并按 (provider, 模型) 缓存，不重复花 |
| **P1** | 抢线窗口 | 工具改写 base_url 到本代理发现之间的短窗口图片可能漏过。接受窗口存在；每次纠正留事件日志；无漂移不写文件 |
| **P1** | GUI/服务生命周期 | 显式开关、关闭确认、托盘（R8） |
| **P1** | 新模型提醒 | 实测自动标注 + 被动通知（不再设批量确认流程；token 代价双向提醒文案） |
...
- 用户自定义回退档案（修复已按崩溃前意图自动推导，无解场景罕见；若仍有需求三期再议）；
- 视频 / 音频模态探测（探针思路相同，本期只做 image）。
...
| 接管组合快照 | 接管时记 base_url + key 位置 + 模型的组合，防外部工具档案污染造成的错位；第二跳归属（CC Switch 也可能路由 Codex）记录为 second_hop，接管态归属判定以它为准（§5 工具档案） |
...
| 模型能力键 | (harness, provider, 模型) 三元组；provider 自动来自工具档案/接管快照；记录只存输入模态三态（纯文本/支持图片/未标注）；取值优先级：用户覆盖 > 实测探测 > 目录建议 > 空 |
| 模态探测 | 可验证探针·纯接收判定（2026-08-25，参考 dsh-image-vision）：200=支持图片、报错含不支持语义=纯文本、含糊（超时/401/404/5xx/格式报错）不下结论回落目录（fnmatch 表+正则启发式）；每模型一次最小调用、按 (provider, 模型) 缓存可重测；`/v1/models` 仅用于拉模型清单（回环 relay 不拉、skipped 带原因）；吞图误标为已知残余风险，user 标注优先兜底 |
| 空状态开关 | 未标注模型的运行行为由全局开关决定（默认走 VLM 转述=安全侧，可切直通）；UI 文案=开关两侧代价说明；外部目录仅作标注建议（只取模态字段，不采上下文/推理）；视频/音频探测留三期 |
