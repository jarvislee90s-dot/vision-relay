...
- **取值优先级：用户覆盖 > 实测探测 > 目录建议 > 未标注（空）**。外部静态目录（models.dev 风格）降级为**标注建议来源**，且只取其输入模态字段。
- **可验证探针（实测探测）**：对每个 (provider, 模型) 用已配 key 发一次最小带图请求——图里藏一个字，问「图里是什么」。三档判定：**报错**（明确拒绝图片输入）→ 纯文本；**答对**（真读到图）→ 支持图片；**返回 200 但答错**（静默吞图，真实存在的坑）→ 纯文本。超时 / 401 / 404 / 5xx 不下结论，回落目录建议。成本为每模型一次最小调用，结果按 (provider, 模型) 缓存、可重测；没配 key 的 provider 显示「未测」。可选「从上游拉取模型清单」（用已配 key 调 `/v1/models`，只补模型 ID 清单）。探针只在该 (provider, 模型) 组合首次出现时自动执行一次，结果作为待确认值推给用户（向导②/模型能力页）——用户过目后无论原样保存还是修改，即为用户意图（source=user），此后不再被自动探测覆盖；重测仅由用户显式触发（2026-08-23 决策）。
